...
def main():
    ...
    file = pd.read_csv('symbols/2022.csv')
    file = file.index.to_list()

    # get all pairs currently available
    all_symbols = pd.DataFrame(requests.get(f'{API_BASE}exchangeInfo').json()['symbols'])
    all_symbols = all_symbols.loc[all_symbols['baseAsset'].isin(file)]
    all_symbols = all_symbols.loc[all_symbols['quoteAsset'].isin(['USDT','BTC','ETC'])] # can change usdt to any other stablecoin
